...
# get user permission
def userpermission(user: str, area: str):
    allowed = False
    msg = ""
    
    if not area.strip().upper().startswith("VIEW"):
        msg = "Area specified must be preceeded with 'View'.  You specified: {area}."
        return allowed, msg
    
    area = f"[{area}]"
    
    try:
        mysql = f"SELECT * FROM tblUserNames WHERE UserName = '{user}' AND {area} = 1;"
        count, msg = getcount(mysql)
        if count > 0:
            allowed = True
